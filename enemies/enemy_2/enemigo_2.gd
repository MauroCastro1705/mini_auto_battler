...
func _spawn_and_travel_coin() -> void:
	# updateado para nuevo resource
	if CoinScript.has_method("spawn_resource1_from_sprite"):
		CoinScript.spawn_resource1_from_sprite(resource_1, global_position, Global.enemigo_resource_1_value)
		return
	...
	var coin_instance: Node = resource_1.duplicate() as Node
