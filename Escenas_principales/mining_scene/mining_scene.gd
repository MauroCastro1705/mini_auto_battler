...
@onready var auto_mining_damage: Label = %auto_mining_damage
...
@onready var auto_mining_timer: Label = %auto_mining_timer
...
func _ready() -> void:
	Global.stats_updated.connect(update_ui)
	update_ui()
...
func update_ui():
	...
	auto_mining_damage.text = str(Global.auto_mining_damage)
	auto_mining_timer.text = str(Global.auto_miner_timer)
